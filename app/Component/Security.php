...
class Security
{
    ...
    protected function encrypt($data, $passwordBased, $secret, $info)
    {
        if (!extension_loaded('openssl')) {
            throw new ServiceErrorException('Encryption requires the OpenSSL PHP extension');
        }
        if (!isset($this->allowedCiphers[$this->cipher][0], $this->allowedCiphers[$this->cipher][1])) {
            throw new ServiceErrorException($this->cipher . ' is not an allowed cipher');
        }
        ...
        if ($encrypted === false) {
            throw new ServiceErrorException('OpenSSL failure on encryption: ' . openssl_error_string());
        }
    ...
    protected function decrypt($data, $passwordBased, $secret, $info)
    {
        ...
        if ($decrypted === false) {
            throw new ServiceErrorException('OpenSSL failure on decryption: ' . openssl_error_string());
        }
    ...
    public function hkdf($algo, $inputKey, $salt = null, $info = null, $length = 0)
    {
        if (function_exists('hash_hkdf')) {
            ...
            if ($outputKey === false) {
                throw new ServiceErrorException('Invalid parameters to hash_hkdf()');
            }
        ...
        if (!$test) {
            throw new ServiceErrorException('Failed to generate HMAC with hash algorithm: ' . $algo);
        }
        ...
        if (!is_int($length) || $length < 0 || $length > 255 * $hashLength) {
            throw new ServiceErrorException('Invalid length');
        }
    ...
    public function pbkdf2($algo, $password, $salt, $iterations, $length = 0)
    {
        if (function_exists('hash_pbkdf2')) {
            ...
            if ($outputKey === false) {
                throw new ServiceErrorException('Invalid parameters to hash_pbkdf2()');
            }
        ...
        if (!is_int($iterations) || $iterations < 1) {
            throw new ServiceErrorException('Invalid iterations');
        }
        if (is_string($length) && preg_match('{^\d{1,16}$}', $length)) {
            $length = (int) $length;
        }
    ...
    public function hashData($data, $key, $rawHash = false)
    {
        ...
        if (!$hash) {
            throw new ServiceErrorException('Failed to generate HMAC with hash algorithm: ' . $this->macHash);
        }
    ...
    {
        if (!is_int($length)) {
            throw new ServiceErrorException('First parameter ($length) must be an integer');
        }

        if ($length < 1) {
            throw new ServiceErrorException('First parameter ($length) must be greater than 0');
        }
        ...
        throw new ServiceErrorException('Unable to generate a random key');
    }
    ...
    public function generatePasswordHash($password, $cost = null)
    {
        ...
        if (!is_string($hash) || strlen($hash) !== 60) {
            throw new ServiceErrorException('Unknown error occurred while generating hash.');
        }
    ...
    public function validatePassword($password, $hash)
    {
        if (!is_string($password) || $password === '') {
            throw new ServiceErrorException('Password must be a string and cannot be empty.');
        }
        ...
        if (!preg_match('/^\$2[axy]\$(\d\d)\$[\.\/0-9A-Za-z]{22}/', $hash, $matches)
            ...
            throw new ServiceErrorException('Hash is invalid.');
        }
    ...
    protected function generateSalt($cost = 13)
    {
        $cost = (int) $cost;
        if ($cost < 4 || $cost > 31) {
            throw new ServiceErrorException('Cost must be between 4 and 31.');
        }
    ...
    public function compareString($expected, $actual)
    {
        if (!is_string($expected)) {
            throw new ServiceErrorException('Expected expected value to be a string, ' . gettype($expected) . ' given.');
        }

        if (!is_string($actual)) {
            throw new ServiceErrorException('Expected actual value to be a string, ' . gettype($actual) . ' given.');
        }
